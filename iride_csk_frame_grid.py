...
from iride_csk_frame_grid_utils import reproject_geodataframe, create_grid_within_polygon, \
                                       add_frame_code_field

def grid_from_area(input_shapefile, output_folder, buffer_dist=None, x_frame_split=3, y_frame_split=6, dissolve=False):
    """
    Process a shapefile, create grids within polygons, and save the result to separate shapefiles.

    Parameters:
        input_shapefile (str): Path to the input shapefile.
        output_folder (str): Path to the output folder where separate grid shapefiles will be saved.
        ...
        dissolve (bool, optional): Whether to dissolve the polygons in the input shapefile.
    ...
    if dissolve and len(gdf['geometry']) > 1:
        ...
        centroid = (dissolved_geometry.centroid.x,dissolved_geometry.centroid.y)
        # Generates the fishnet grid of input frame polygons
        grid_gdf = create_grid_within_polygon(dissolved_geometry, x_frame_split, y_frame_split)
        ...
        for idx, geometry in enumerate(gdf['geometry']):
            grid_gdf = create_grid_within_polygon(geometry, x_frame_split, y_frame_split)
